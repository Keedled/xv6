...
binit(void)
{
  ...
  for(b = buf; b < buf + NBUF; b++){
    ...
    int h = bhash(b->dev, b->blockno); // 初始一般都是 0
    b->next = bucket[h].head.next;
    b->prev = &bucket[h].head;
    bucket[h].head.next->prev = b;
    bucket[h].head.next = b;
  }
...
bget(uint dev, uint blockno)
{
  ...
  acquire(&bucket[h].lock);

  for(b = bucket[h].head.next; b != &bucket[h].head; b = b->next){
    if(b->dev == dev && b->blockno == blockno){
      b->refcnt++;
      release(&bucket[h].lock);
      acquiresleep(&b->lock);
      return b;
    }
  }
